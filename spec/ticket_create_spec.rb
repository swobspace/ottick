...
describe 'TicketCreate' do
  describe "with valid settings", modify: true do
    ...
    it "ticket_create returns some content" do
      ...
      options = { "Ticket" => { "CustomerUser" => customer_user } }

      response = @ticket.ticket_create(subject, text, options)
      ...
      expect(response.body[:ticket_get_response][:ticket][:customer_user_id]).to eq(customer_user)
      expect(response.body[:ticket_get_response][:ticket][:customer_id]).to eq(customer_id)
    end
